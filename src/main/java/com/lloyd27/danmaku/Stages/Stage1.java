...
public class Stage1 extends AbstractStageGame{
    ...
    public void update(double deltaTime) {
        ...
        // Gestion de la pause (bouton echape ou mort du joueur)
        if (pause) {
            gestionPause(deltaTime);
        }else{
            // variables qui garde le temp ecoulée
            ...
            gestionPhaseStage(deltaTime);

            gestionTirEnemy();

            gestionAutowiredPlayer();
            ...
            addToRemoveOffScreen(toRemove,deltaTime);

            gestionDmgBullet();

            gestionColisionJoueurEnemy();

            gestionSupressionEntity(toRemove);

            gestionAvantSupressionBoss(toRemove);

            gestionAvantSupressionBomb(toRemove);

            // Supression des entités a supprimer    
            entity.removeAll(toRemove);
        }
    }

    // Gestion des différentes phases du stage1
    private void gestionPhaseStage(double deltaTime){
            ...
            }else if(spawnEnemy5 && timeSinceStart>25 && timeSinceStart<37){
                spawnEnemy5();
            }else if(spawnEnemy6 && timeSinceStart>37 && timeSinceStart<48){
                spawnEnemy6();
                spawnEnemy5=true;
            }else if (timeSinceStart >= 52 && timeSinceStart <=60) {
                spawnEnemy3(timeSinceLastSpawn); 
                spawnKunai2(timeSinceLastSpawnKunai3);
            }else if((timeSinceStart>65 && timeSinceStart<95)){
                spawnEnemy7();
            ...
            }else if (timeSinceStart > 129  && bossSpawn == false) {
                    entity.removeIf(e -> e instanceof AbstractEnemyShooter);
                    spawnBoss();
            }
    }
    ...
    public void render(GraphicsContext gc) {
        ...
        // décalage des 2 background pour effet de continuité
